...
if __name__ == "__main__":
    ...
    if "config" in args and len(args) == 1:
        ...
        cmd_info = """Commands
        --------
        `%run -m manimsetup install` — Manim complete install
        `%run -m manimsetup -l install` — Manim without LaTeX install
        `%run -m manimsetup config` — Manim import and config
        `%run -m manimsetup -h` — Command information
        """
        print(cmd_info)
